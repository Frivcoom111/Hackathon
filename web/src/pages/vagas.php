...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['vaga_id'])) {
    ...
    $vagaId = trim($_POST['vaga_id']);
    $data = api_post_json('/jobs/' . urlencode($vagaId) . '/apply', [], $_SESSION['token']);

    if ($data['success'] ?? false) {
        $msgCandidatura = 'Candidatura enviada com sucesso!';
    } else {
        $erroCandidatura = $data['message'] ?? 'Nao foi possivel enviar a candidatura.';
    }
}
...
$jobs = api_items(api_get('/jobs?status=ACTIVE'), 'jobs');
if ($jobs === []) {
    $jobs = demo_jobs();
}
...
foreach ($jobs as $item) {
    ...
    if ($busca && stripos($vaga->getTitulo(), $busca) === false) {
        continue;
    }

    if ($cidade && stripos($vaga->getLocalizacao(), $cidade) === false) {
        continue;
    }

    if ($area && stripos($vaga->getArea(), $area) === false) {
        continue;
    }

    if ($bolsaMin > 0 && ($vaga->getSalario() ?? 0) < $bolsaMin) {
        continue;
    }

    if (!in_array($vaga->getModalidade(), (array)$modalidades, true)) {
        continue;
    }
...
<section class="vagas-hero">
  <div class="container">
    <h1 class="vagas-hero-titulo">Vagas de Estagio</h1>
      ...
      <aside class="vagas-filtros">
        <form method="GET" action="<?= BASE ?>index.php" id="form-filtros">
          ...
          <div class="filtro-grupo">
            ...
            <?php
            $mods = ['PRESENCIAL' => 'Presencial', 'REMOTE' => 'Remoto', 'HYBRID' => 'Hibrido'];
            foreach ($mods as $val => $label):
                $checked = in_array($val, (array)$modalidades, true) ? 'checked' : '';
            ?>
          ...
          <div class="filtro-grupo">
            ...
            <select class="form-select form-select-sm" name="area">
              ...
              <?php
              $areas = ['Tecnologia', 'Administracao', 'Marketing', 'Direito', 'Saude', 'Educacao'];
              foreach ($areas as $label):
              ?>
                <option value="<?= htmlspecialchars($label) ?>" <?= $area === $label ? 'selected' : '' ?>>
          ...
          <div class="filtro-grupo">
            <label class="filtro-label">Bolsa minima</label>
      ...
      <div class="vagas-lista-conteudo">
        ...
        <?php if (empty($vagas)): ?>
          <div class="vagas-vazio">
            ...
            <a href="<?= BASE ?>index.php?page=vagas" class="btn btn-outline-primary btn-sm">Limpar filtros</a>
          </div>
        <?php else: ?>
          <div class="row g-4">
              ...
              <div class="col-lg-6 col-12">
                <div class="vaga-card">
                  <div class="vaga-card-top">
                    ...
                    <span class="vaga-badge">Estagio</span>
                  </div>
